// Package parse implements multi-protocol proxy subscription and tunnel decoders.
// Source: https://pkg.go.dev/crypto/rsa (RSA PKCS#1 v1.5 block decryption)
package parse

import (
	"crypto/rsa"
	"crypto/x509"
	"encoding/base64"
	"errors"
	"fmt"
	"regexp"
	"strings"
	"sync"
)

// HTTPCustomPKCS1KeysB64 stores the embedded RSA private keys for crypt1 through crypt4.
var HTTPCustomPKCS1KeysB64 = []string{
	// happ://crypt/   (RSA-1024)
	"",
	// happ://crypt2/  (RSA-4096)
	"",
	// happ://crypt3/  (RSA-4096)
	"",
	// happ://crypt4/  (RSA-4096)
	"",
}

var (
	happKeyOnce sync.Once
	happKeys    map[string]*rsa.PrivateKey
	happRegex   = regexp.MustCompile(`^(?:happ://)?([^/]+)/(.+)$`)
)

func initHTTPCustomKeys() {
	happKeys = make(map[string]*rsa.PrivateKey)
	versionMap := []string{"crypt", "crypt2", "crypt3", "crypt4"}

	for idx, b64Raw := range HTTPCustomPKCS1KeysB64 {
		if idx >= len(versionMap) {
			break
		}
		derBytes, err := base64.StdEncoding.DecodeString(b64Raw)
		if err != nil {
			continue
		}
		priv, err := x509.ParsePKCS1PrivateKey(derBytes)
		if err != nil {
			key, err := x509.ParsePKCS8PrivateKey(derBytes)
			if err != nil {
				continue
			}
			if rsaKey, ok := key.(*rsa.PrivateKey); ok {
				priv = rsaKey
			}
		}
		if priv != nil {
			happKeys[versionMap[idx]] = priv
		}
	}
}

// DecryptHTTPCustom decrypts an HTTP Custom link or configuration payload.
func DecryptHTTPCustom(link string) (string, error) {
	if strings.TrimSpace(link) == "" {
		return "", errors.New("http_custom: empty link")
	}

	happKeyOnce.Do(initHTTPCustomKeys)

	matches := happRegex.FindStringSubmatch(link)
	if matches == nil {
		return "", errors.New("http_custom: invalid link format")
	}

	version := matches[1]
	encryptedData := matches[2]

	keysToTry := []string{version, "crypt", "crypt2", "crypt3", "crypt4"}

	for _, kVer := range keysToTry {
		priv, exists := happKeys[kVer]
		if !exists || priv == nil {
			continue
		}

		decrypted, err := decryptRSAChunked(encryptedData, priv)
		if err == nil && len(decrypted) > 0 {
			return decrypted, nil
		}
	}

	return "", errors.New("http_custom: failed to decrypt with any available rsa key")
}

func decryptRSAChunked(encryptedB64 string, priv *rsa.PrivateKey) (string, error) {
	if encryptedB64 == "" {
		return "", errors.New("http_custom: empty encrypted payload")
	}

	s := strings.ReplaceAll(encryptedB64, "-", "+")
	s = strings.ReplaceAll(s, "_", "/")
	switch len(s) % 4 {
	case 2:
		s += "=="
	case 3:
		s += "="
	}

	cipherBytes, err := base64.StdEncoding.DecodeString(s)
	if err != nil {
		return "", fmt.Errorf("http_custom: base64 decode failed: %w", err)
	}

	keySize := (priv.N.BitLen() + 7) / 8
	if len(cipherBytes)%keySize != 0 {
		return "", errors.New("http_custom: ciphertext not aligned to rsa block size")
	}

	var plaintext []byte
	for i := 0; i < len(cipherBytes); i += keySize {
		chunk := cipherBytes[i : i+keySize]
		decryptedChunk, err := rsa.DecryptPKCS1v15(nil, priv, chunk)
		if err != nil {
			return "", fmt.Errorf("http_custom: rsa decrypt chunk failed: %w", err)
		}
		plaintext = append(plaintext, decryptedChunk...)
	}

	return string(plaintext), nil
}

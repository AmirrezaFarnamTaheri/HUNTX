...
class TestAmbiguousOutcome(unittest.TestCase):
    def test_transport_failure_is_not_retried(self):
        publisher = TelegramPublisher("123:secret")
        with patch(
            "huntx.publishers.telegram.publisher.urllib.request.urlopen",
            side_effect=urllib.error.URLError("connection reset"),
        ) as urlopen:
            with self.assertRaises(UnknownPublicationOutcome):
                publisher.publish("1", b"payload", "artifact.txt")

...
class TestRawStoreHashValidation(unittest.TestCase):
    ...
    def test_corrupt_existing_blob_is_repaired_on_save(self):
        payload = b"trusted content"
        digest = self.store.save(payload)
        blob = Path(self._tmp.name) / digest[:2] / digest
        blob.write_bytes(b"tampered")

        self.assertEqual(self.store.save(payload), digest)
        self.assertEqual(blob.read_bytes(), payload)

    def test_corrupt_blob_is_never_returned_as_valid(self):
        digest = self.store.save(b"trusted content")
        blob = Path(self._tmp.name) / digest[:2] / digest
        blob.write_bytes(b"tampered")

        self.assertFalse(self.store.exists(digest))

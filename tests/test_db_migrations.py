...
class TestDBMigrations(unittest.TestCase):
    ...
    def test_migration_adds_metadata_json(self):
        ...
        conn.execute("CREATE TABLE seen_files (id INTEGER PRIMARY KEY, source_id TEXT, external_id TEXT, raw_hash TEXT, file_size INTEGER, filename TEXT, status TEXT, error_msg TEXT)")
        conn.commit()
        conn.close()

...
class TestOpaqueBundleCollisionPerformance(unittest.TestCase):
    ...
    def test_collision_handling_is_not_quadratic(self):
        ...
        small_records = make_records(small)
        large_records = make_records(large)

        with patch.object(self.store, "get", return_value=b"payload"):
            start = time.monotonic()
            self.handler.build(small_records)
            small_elapsed = time.monotonic() - start

            start = time.monotonic()
            self.handler.build(large_records)

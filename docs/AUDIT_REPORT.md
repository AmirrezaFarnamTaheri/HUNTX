...
- **Remediation:** Periodic orphan sweep (walk shards, LEFT JOIN against seen_files); use `atomic_write` for the manifest. **Effort:** S. **Priority:** P2. **Validation:** Inject orphan; sweep removes it; kill during manifest write leaves valid prior file.

### F-14 — f-string SQL construction in `_get_protocol_counts` 🟡 MEDIUM
- **Evidence:** `interactive.py:726–743` interpolates scheme constants into `LIKE '...'` CASE arms.
- **Impact:** Not injectable today (constants), but a single refactor to dynamic schemes creates SQLi in the user-facing bot; violates the codebase's otherwise-consistent parameterization.
- **Remediation:** Parameterize (`LIKE ? || '%'`) or precompute protocol in `records` at insert. **Effort:** S. **Priority:** P2. **Validation:** Code scan (semgrep rule) passes.

### F-15 — `_PROXY_SCHEMES` quadruplicated across modules 🟡 MEDIUM
- **Evidence:** `core/router.py`, `pipeline/build.py`, `formats/npvt.py`, `scripts/verify_output.py` each define the tuple; bot imports the npvt copy.
- **Impact:** Scheme additions silently miss decoders or detection → records ingested but never decoded, or vice versa (drift class bug).
- **Remediation:** Single constants module. **Effort:** S. **Priority:** P2. **Validation:** One definition site; tests import it.

### F-16 — Bot API connector's class-level update cache is unbounded and process-global 🟡 MEDIUM
- **Evidence:** `TelegramConnector._shared_state[token]["updates"]` only grows; offsets consumed once per process.
- **Impact:** Memory leak in long-lived processes; cross-instance coordination relies on dict mutation from multiple threads without a lock (GIL-tolerable but unguaranteed ordering).
- **Remediation:** Evict updates older than max processed offset; add a class lock. **Effort:** S. **Priority:** P3 (no Bot API sources in prod config today).

### F-17 — Derived-artifact hashes are suffix-decorated base hashes 🟡 MEDIUM
- **Evidence:** `build.py`: `"artifact_hash": artifact_hash + "_dec"` / `"_b64"`.
- **Impact:** Change-detection blind to decoder changes; semantically a content hash that isn't. **Remediation:** Hash derived bytes. **Effort:** S. **Priority:** P3.

### F-18 — StateRepo swallows read errors into benign defaults 🟡 MEDIUM
- **Evidence:** `get_source_state`, `has_seen_file`, `get_records_for_build`, `get_processed_hashes` catch-all → `None/False/[]`.
- **Impact:** A corrupted DB reads as "fresh world": sources refetch everything (has_seen_file False → duplicate ingestion blocked only by raw-hash CAS), builds emit empty artifacts, prune treats nothing as processed. Failures masquerade as quiet emptiness.
- **Remediation:** Distinguish corruption (raise) from absence (default); add a startup `PRAGMA integrity_check`. **Effort:** S–M. **Priority:** P2.

### F-19 — Lock contention exits with code 0; unlock attempted when never acquired 🟡 MEDIUM
- **Evidence:** `locks.py`: `except (IOError, OSError): … sys.exit(0)`; `finally` unlocks regardless.
- **Impact:** Skipped runs look successful to CI/systemd (no alert on a wedged previous process); on the failure path, unlocking an unheld region raises (caught broadly).
- **Remediation:** Exit non-zero (or a distinct code CI maps to "skipped"); track acquisition state. **Effort:** S. **Priority:** P2.

### F-20 — v2ray_collector requires a Go toolchain absent from CI; generated outputs committed into the package tree 🟡 MEDIUM
- **Evidence:** `subprocess.run(["go", "run", "main.go"])`; workflow installs only Python; `collected_configs.txt`, `ir_configs.txt`, `irb64.txt`, `last_update.txt` tracked inside `src/huntx/connectors/v2ray_collector/`, and the connector deletes/rewrites `collected_configs.txt` at runtime → dirty working tree inside the installed package, polluting CI git state and the sdist.
- **Remediation:** Gate the source type on `shutil.which("go")` with a clear error; gitignore generated files; precompile the binary or vendor outputs out of `src/`. **Effort:** S. **Priority:** P3 (unused in prod) / P1 the day someone enables it.

### F-21 — Ad-hoc migrations: one hard-coded column check, no schema version 🔵 LOW
- **Evidence:** `db.py._check_migrations` checks only `seen_files.metadata_json`.
- **Remediation:** `schema_version` table + ordered migration list (tests `test_db_migrations.py` exist — extend the mechanism they cover). **Priority:** P3.

### F-22 — Repo history bloat from artifact commits 🔵 LOW
- **Evidence:** Dozens of "Update outputs [skip ci]" commits; `docs/artifacts/` re-snapshotted per run (incl. archive copies).
- **Remediation:** Branch/Release publishing (see F-07); BFG-clean history if size becomes pressing. **Priority:** P3.

### F-23 — Missing env vars expand to `""` producing opaque pydantic failures ×85 🔵 LOW
- **Evidence:** `env_expand.py` substitutes absent vars with `""`; `api_id: int` then fails validation for every source.
- **Remediation:** Fail fast with a named-variable error before model validation. **Priority:** P3. (Also fixes the broken README quick-start path, F-26/§11.)

### F-24 — Publisher parses `retry_after` but never retries 🔵 LOW
- **Evidence:** `publisher.py:78–84` logs the rate limit then raises.
- **Impact:** Pipeline-level retry happens only on the *next run* (2 h later). **Remediation:** Sleep-and-retry once for small `retry_after`. **Priority:** P3.

### F-25 — No dependency lockfile; actions pinned by tag 🔵 LOW (amplifier for F-07)
- **Remediation:** `pip-compile` with hashes; SHA-pin actions; Dependabot. **Priority:** P2 given F-07 coupling.

### F-26 — "Quality Gates" claim vs E9-only flake8 🔵 LOW
- **Remediation:** Enable full flake8 ruleset already configured in `.flake8`/pyproject (line-length 120 exists but isn't enforced in CI), or amend README. **Priority:** P3.

### F-27 — `verify_output.py` optionally executes `v2ray test` on a config aggregated from untrusted channel content 🔵 LOW
- **Evidence:** lines 268–276. A v2ray/xray config parser CVE would be triggerable by channel operators inside CI (with secrets in env).
- **Remediation:** Drop the exec, or run in a separate secretless job/container. **Priority:** P3 (binary not installed on runners today → dormant).

### F-28 — Frontend loads all JS deps from CDNs without SRI 🔵 LOW
- **Evidence:** `docs/index.html` (Tailwind CDN, esm.sh, Fuse, Lucide).
- **Impact:** CDN compromise = arbitrary script on the project's public page (no credentials there, defacement/SEO risk). **Remediation:** Vendor + SRI. **Priority:** P3.

---

# 13. Risk Register (prioritized)

| Rank | Risk | Findings | Likelihood | Impact | Owner suggestion |
|---|---|---|---|---|---|
| 1 | Public outputs stale/wrong | F-01 | Certain | Product-breaking | Pipeline owner |
| 2 | Telegram session revocation → total ingestion outage | F-02, F-25 | High | Total outage, manual recovery | Platform owner |
| 3 | Run >3 h loses all state, thrash loop | F-06, F-03 | Medium | Recurrent data loss | CI owner |
| 4 | Supply-chain compromise with repo-write + secrets | F-07, F-25 | Medium | Repo + accounts compromise | Security |
| 5 | Malicious content redistributed to users | F-08 | Medium | User harm, platform bans | Product |
| 6 | Legal exposure from embedded vendor keys | F-04 | Certain (presence) | Deal/compliance risk | Legal/Eng |
| 7 | Admin takeover via username churn | F-05 | Low-Med | Full bot/pipeline control | Eng |
| 8 | Silent total-failure runs (green CI, zero data) | F-18, F-19, orchestration exit-0 policy | Medium | Slow-burn data gaps | Ops |
| 9 | Subscriber base wiped by maintenance reset | F-11 | Low | Irreversible audience loss | Ops |
| 10 | Disk/repo growth (orphans, artifacts-in-git, manifest) | F-13, F-22, §9 | High (slow) | Operational decay | Ops |
...
|---|---|---|---|---|
| P0 | Reconcile output paths; stop committing artifacts to `main` (branch/Release instead) | F-01, F-07, F-22 | S–M | Fresh catalog timestamps; clean main history |
| P0 | Re-architect MTProto usage: single client per process, async iteration; separate bot session; remove cross-method locks | F-02, F-10 | M–L | Soak + chaos tests; no AUTH_KEY warnings |
| P1 | Per-batch commits in ingest; pass deadline to ingest; cancel workers on timeout | F-03, F-12 | S | Crash/timeout tests |
| P1 | Align timeouts (orchestrator ≤ 150 min); `if: always()` persist step | F-06 | S | Forced-slow run persists state |
| P1 | Numeric-ID admin auth; bot rate limiting; ZIP-content scanning; distinct-token assertion | F-05, F-08, F-09 | M | Unit + load tests |
| P1 | Externalize vendor keys; secret-scan CI gate | F-04 | S–M | gitleaks clean |
| P2 | Lockfile + SHA-pinned actions; least-privilege publish job | F-25, F-07 | M | Reproducible builds |
| P2 | Hardening pass: parameterized CASE SQL, single `_PROXY_SCHEMES`, atomic manifest, orphan sweep, fail-fast env expansion, non-zero lock exit, corruption-vs-absence in StateRepo, separate bot DB | F-11, F-13–F-15, F-18, F-19, F-23 | M | Targeted unit tests (the suite structure already supports this) |
| P3 | Schema versioning; publisher 429 retry; README truth pass; frontend SRI; Go-toolchain gating; remove duplicate site-data step | F-16, F-17, F-20, F-21, F-24, F-26–F-28 | M | Doc review + smoke tests |

---

# 15. Strategic Recommendations

**Short term (≤1 month):** Execute P0/P1 above. Add a single "run health" gate: fail (or alert) the workflow when `results["ok"] == 0` or zero artifacts were produced — today total failure is green. Decide the artifact-distribution story once (Releases or Pages, not git-on-main).

**Medium term (1–3 months):** Replace the thread-pool-of-event-loops with one asyncio orchestrator (Telethon-native), which simultaneously fixes F-02's structure, removes the lock machinery, and makes FloodWait handling global. Introduce a `schemes.py`/`constants.py` core module. Add minimal telemetry: per-run JSON summary committed/uploaded, with a tiny status badge on the Pages site (the frontend already exists to host it). Split `interactive.py` into `bot/{handlers,delivery,admin,stats}.py`.

**Long term (3–12 months):** If the project grows beyond hobby scale: move state from round-tripped SQLite to a hosted Postgres (or at minimum Litestream-style continuous replication), move the bot to a small always-on host as the *only* pipeline trigger (eliminating the CI/bot duality), and implement endpoint health-checking before publication (the Go collector's latency tester is a seed for this — promote it rather than letting it rot as an unused subprocess). Resolve the policy posture explicitly (Telegram ToS, GitHub AUP, key-material legality) before any commercialization or transfer.

---

# 16. Evidence Appendix (claim → location map)

| Claim | Evidence location |
|---|---|
| 85 telegram_user sources, env-templated session | `configs/config.prod.yaml` (85× `${TELEGRAM_USER_SESSION}`) |
| CI commits repo-root outputs | `.github/workflows/huntx.yml:226–246` |
| Pipeline writes data-dir outputs | `store/paths.py:18–19,57–58`; `core/orchestrator.py:83,178`; CI `--data-dir persist/data` |
| Site script scans repo-root `outputs_dev` | `scripts/generate_site_data.py:19` |
| Per-thread clients on one StringSession; cross-method lock | `connectors/telegram_user/connector.py:46–92,500–525` |
| Bot-triggered pipeline run sharing session/state | `bot/interactive.py:979–1014` |
| Whole-source single transaction | `pipeline/ingest.py` `run()` (`with … connect() as conn:` spanning iteration) |
| 4.5 h orchestrator timeout vs 180 min job | `cli/main.py:117`; workflow `timeout-minutes: 180` |
| RSA private keys / vendor passwords | `formats/common/crypto.py:18–140,247–252,300` |
| Username admin auth | `bot/interactive.py:341–349` |
| f-string SQL CASE | `bot/interactive.py:726–743` |
| Daemon threads + timeout abandonment | `core/orchestrator.py:450,460–467,475–477` |
| Exit-0 on lock contention | `core/locks.py:24–26` |
| Derived hash suffixing | `pipeline/build.py` (`artifact_hash + "_dec"/"_b64"`) |
| getUpdates polling + token fallback | `connectors/telegram/connector.py:151–185`; `pipeline/publish.py:67`; `cli/main.py:286` |
| Non-atomic manifest write | `core/orchestrator.py:221` vs `utils/atomic.py` |
| Minimal lint gate | workflow `flake8 … --select=E9,F63,F7,F82` |
| Go subprocess + committed generated files | `connectors/v2ray_collector/connector.py:49–62`; tracked `collected_configs.txt` etc. |
| v2ray exec on untrusted config | `scripts/verify_output.py:268–276` |
| ZIP passthrough rationale | `utils/content_type.py:40–55` |
| Publisher parses retry_after, no retry | `publishers/telegram/publisher.py:75–84` |
| Token masking / no-token-in-URL logging discipline | `pipeline/publish.py:84`; `connectors/telegram/connector.py:96`; `publisher.py:60` |
...
- CI is the canonical deployment (the systemd unit suggests a secondary persistent-bot host whose configuration was not observable from the repo).
- GitHub secrets are populated as the workflow expects; we cannot verify their values or whether `PUBLISH_BOT_TOKEN` is distinct from `TELEGRAM_TOKEN` (F-09's likelihood depends on this).
- Repo-root `outputs/` staleness is inferred from path arithmetic plus the absence of any copying step; live CI logs were not available to this audit. Confidence remains High but a single CI log would make it conclusive.

**Unknowns:**
- Actual data volumes (records count, blob store size, user count) — S3 state and runtime artifacts were not accessible.
- Telethon behavioral specifics under this exact lock pattern (whether Telegram tolerates short-lived sequential reconnects of one auth key from one IP — empirically it often does, which may explain survival to date; F-02 is about *fragility*, not certainty of past failure).
- `docs/USER_GUIDE.md` / `docs/DEVELOPMENT.md` full-content accuracy (sampled, not exhaustively diffed).
- Legal jurisdiction context for F-04 and the censorship-circumvention use case.
- Whether `data_archive/` or other untracked local directories exist in deployments (not in git).

---

# 18. Final Verdict

**Engineering quality: B-.** The pipeline core (router → handlers → batch transform → CTE-deduplicated build → change-detected publish) is well-designed, defensively coded, and unusually well-tested for its genre (29 test modules covering migrations, concurrency, hardening, and per-format behavior). Logging and incremental-state design are professional-grade.

**Production readiness: C-.** Two structural faults dominate: the storage-layout refactor that disconnected CI's publication step from the pipeline's actual outputs (F-01) means the system's public face cannot currently be trusted; and the shared-session threading model (F-02) means the system's single most valuable credential lives in a perpetual race condition, with no automated recovery path. Both are fully fixable with small-to-medium effort, and neither reflects careless code — they reflect missing integration-level verification (nothing tests "does the committed output match the built output?", "does parallel ingestion actually parallelize?").

**Security posture: C.** No first-party secrets leaked; consistent parameterized SQL (one exception), path-traversal guards, token-masking discipline, magic-byte filtering — all better than baseline. Offset by embedded third-party private keys, a spoofable admin gate, an over-privileged cron job running unpinned dependencies, and the irreducible fact that the product redistributes unverified attacker-supplied configuration to end users.

**Acquisition/diligence view:** This is a competent solo-engineered system whose value is the pipeline framework and format-handler library, not the operational deployment. A buyer should budget ~2–4 engineer-weeks of P0/P1 remediation, a legal review of `crypto.py` provenance, and a platform-risk acceptance for the Telegram/GitHub dependency before relying on it. With those done, the architecture has clear headroom to 5–10× its current source count after the asyncio migration; without them, expect the first major incident to be either a silent month of stale public outputs (already plausibly in progress) or a revoked Telegram session taking ingestion to zero in one moment.

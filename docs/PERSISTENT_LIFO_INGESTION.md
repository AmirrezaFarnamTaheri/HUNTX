...
Only fully closed windows are seeded. The anchor is the most recent aligned boundary at or before the current time, and no persisted work item may end after that anchor. The active partial window is deliberately excluded and becomes eligible after it closes. This prevents a partially observed active window from being marked permanently complete before later messages arrive.

When the requested lookback is not divisible by the window size, HuntX rounds the start outward to the previous aligned boundary. The resulting windows cover at least the requested horizon, remain aligned, and never extend beyond the closed-window anchor.

Telethon is queried newest-to-oldest with exclusive `offset_date=window_end_ts`. The lower boundary is enforced in application code. Continuation uses exclusive `offset_id`: when a page ends on message ID `N`, the next page starts with messages older than `N`.

## Ordering contract

1. The newest incomplete closed window is selected first.
2. Every eligible MTProto source is processed within that window before an older window may be claimed.
3. Sources rotate after each bounded page using a durable `rotation_seq`, so sub-second checkpoints cannot cause one source to immediately reclaim the queue.
4. Newly closed windows inserted by a later scheduled run preempt older residue because the queue is strict LIFO by `window_end_ts`.
5. Telegram Bot API sources remain live-only because `getUpdates` cannot retrieve historical channel messages.

The strict barrier intentionally prevents workers from descending into older history while the newest window is leased or waiting for a transient retry. Removed sources and duplicate aliases are terminalized as `quarantined`, so stale work cannot block the barrier indefinitely.

## Durable residue

Each source/window pair is stored once in `ingestion_work_items` with a global uniqueness constraint:
...
- **Crash after commit:** observations and cursor are both durable, and the row is already `partial` or `completed`.
...
- **Removed or renamed source:** incomplete work is marked `quarantined` and excluded from the strict barrier.
...
Before seeding, HuntX resolves each MTProto source to its canonical Telegram channel ID. Only the first configured source for a canonical channel is admitted. Aliases are not seeded, and any legacy incomplete rows for those aliases are terminalized. If canonical resolution fails, the source is preserved rather than silently discarded.

This complements the independent idempotency layers:

- **Work deduplication:** one durable row per source/window.
...
No manual migration command is required. `DBConnection` applies `schema.sql` at startup with `CREATE TABLE IF NOT EXISTS`, creating the campaign and work-item tables alongside the existing state tables. Schema migration version 2 adds the durable `rotation_seq` column to databases created by the first PR revision. Existing `source_state` offsets, `seen_files`, records, and publication history are retained.

On first execution, the queue seeds source/window work rows for the configured horizon. Existing observations are skipped by the current `seen_files` uniqueness constraint, making the transition from source offsets idempotent.
...
When the ingestion budget expires, workers stop claiming pages, release unfinished leases to residue, and continue through transform, build, publication, export, and durable-state persistence. `ingest_skipped_due_to_budget` reports residue only when the ingestion budget was actually exhausted; otherwise it is zero and the complete residue remains available under `lifo_residue`.

## Configuration

| Environment variable | Default | Recommended use |
|---|---:|---|
| `HUNTX_LIFO_LOOKBACK_HOURS` | `file_fresh_hours` (normally `48`) | Keep at `48` for the production acquisition horizon; non-finite values fall back safely |
...
`lifo_residue.remaining` is the number of pending, partial, leased, or retry-wait work items that will survive for another session. Terminal `quarantined` rows remain visible in the status summary but are not counted as residue and do not block older windows.

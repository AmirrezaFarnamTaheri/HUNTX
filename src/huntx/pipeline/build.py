...
_STANDARD_PROTOCOLS = {'vless': 'vless', 'trojan': 'trojan', 'hysteria2': 'hysteria2', 'hy2': 'hysteria2', 'hysteria2+realm': 'hysteria2', 'hysteria2+realm+http': 'hysteria2', 'hysteria': 'hysteria', 'tuic': 'tuic', 'wireguard': 'wireguard', 'wg': 'wireguard', 'socks': 'socks', 'socks5': 'socks', 'socks4': 'socks', 'socks4a': 'socks', 'http': 'http', 'https': 'http', 'ssh': 'ssh', 'shadowtls': 'shadowtls', 'naive+https': 'naive', 'naive+quic': 'naive', 'anytls': 'anytls', 'juicity': 'juicity', 'mierus': 'mieru', 'warp': 'warp', 'dns': 'dns', 'dnstt': 'dnstt'}
_DERIVED_PROXY_FORMATS = frozenset({'npvt', 'npvtsub'})


class BuildPipeline:

    def __init__(self, state_repo: StateRepo, artifact_store: ArtifactStore, registry: FormatRegistry, *, format_locks: Optional[dict[str, threading.Lock]] = None, format_locks_guard: Optional[threading.Lock] = None) -> None:
        """Initialize the pipeline and its shared synchronization primitives."""
    ...
    def _format_lock(self, format_id: str) -> threading.Lock:
        """Return the per-format build lock, creating it lazily."""
    ...
    def _parse_standard_uri(line: str, protocol: str) -> dict[str, Any]:
        """Decode common URI components without letting exotic ports abort decoding.

        Hysteria2 multi-port links are intentionally tolerated here: the richer
        sing-box parser handles their exact port ranges, while decoded.json still
        records the protocol/raw URI even when urllib cannot expose ``.port``.
        """
        result: dict[str, Any] = {'protocol': protocol, 'raw': line}
        try:
        ...
        try:
            ...
            result['port_spec'] = parsed.netloc.rsplit('@', 1)[-1]
    ...
    def _decode_ss(line: str) -> dict[str, Any]:
        """Decode one Shadowsocks share URI into structured metadata."""
        try:
            rest = line[5:]
            tag = ''
            if '#' in rest:
            ...
            if '@' in rest:
                ...
                return {'protocol': 'shadowsocks', 'method': method, 'password': password, 'address': host, 'port': int(port_s), 'tag': tag, 'raw': line}
    ...
    def _decode_ssr(line: str) -> dict[str, Any]:
        """Decode one ShadowsocksR share URI into structured metadata."""
        try:
    ...
    def _decode_single_line(self, line: str) -> Optional[dict[str, Any]]:
        """Decode one supported proxy share line or return None."""
        ...
        if protocol is None:
            return None
        return self._parse_standard_uri(line, protocol)

    def _decode_proxy_text(self, text: str) -> bytes:
        """Decode supported proxy lines and aggregate protocol counts."""
        ...
        if not decoded_entries:
            return b''
        payload = {'total': len(decoded_entries), 'protocols': protocols, 'entries': decoded_entries}
        return json.dumps(payload, indent=2, ensure_ascii=False).encode('utf-8')

    def _decode_proxy_links(self, artifact_bytes: bytes) -> bytes:
        """Decode a UTF-8 proxy artifact into diagnostic JSON bytes."""
        try:
    ...
    def _reencode_as_base64_sub(artifact_bytes: bytes) -> bytes:
        """Encode a textual proxy artifact as a base64 subscription."""
        try:
    ...
    def run(self, route_config: dict[str, Any], *, records: Optional[list[dict[str, Any]]] = None, deadline: Deadline | None = None) -> list[dict[str, Any]]:
        ...
        logger.info('[Build] route=%s fetched=%s fetch_seconds=%.3f types=%s', route_name, record_count, fetch_duration, type_counts)
        if not records:
            return []

...
def main():
    ...
    parser.add_argument(
        "--data-dir",
        default=os.getenv("HUNTX_DATA_DIR", "./data"),
        help="Data directory",
    )
    parser.add_argument(
        "--db-path",
        default=None,
        help="State DB path (overrides HUNTX_STATE_DB_PATH)",
    )
    ...
    db_path = args.db_path or os.getenv("HUNTX_STATE_DB_PATH")
    if db_path is None:
        db_path = str(Path(args.data_dir) / "state" / "state.db")
